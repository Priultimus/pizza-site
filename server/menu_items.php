...
function fetchMenuItemReviews($menuItemID) {
    $query = "SELECT * FROM reviews WHERE itemID = $menuItemID";
    global $db;
    $result = mysqli_query($db, $query);
    $reviews = [];
    while ($row = mysqli_fetch_assoc($result)) {
...
function returnWholeReviewElement($itemId) {
    echo "<div class='menu-card-reviews'>
            <h1 class='reviews-title'>ITEM REVIEWS</h1>
              <div class='reviews'>
                <form class='review create-review-card' method='GET'>
                ...
                foreach (fetchMenuItemReviews($itemId) as $review) { echo $review; }
              echo "</div>
          </div>";
}
...
function fetchMenuItemsByCategory($category) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $menuItems[$id] = "<div class='menu-card'>
                <div class='menu-card-panel'>
                  <img class='menu-card-image' src='$image' alt='$desc' />
                  <div class='menu-card-details'>
                    <h3>$name</h3>
