...
// Adds an item to the user's cart and sends it to the database.
function addToCart(itemId){
    let items = []

    fetch('../server/cart.php?itemId=${itemId}')
    ...
    .then((json) => {
        ...
        // Send a POST request to update the user's database cart with the new item.
        fetch('../server/cart.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ cartJson: items }),
        })
        .then((response) => {
            if (!response.ok) {
...
// Remove an item from the user's cart and send it to the database.
function removeFromCart(itemId){
    ...
    .then((json) => {
        ...
        // Check for any matching item IDs and remove the first occurence.
